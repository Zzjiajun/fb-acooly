...
<#--&lt;#&ndash;                <input type="text" class="form-control form-control-sm" name="phoneNumber"/>&ndash;&gt;-->
<#--                <textarea rows="12" cols="80"  name="phoneNumber" class="easyui-validatebox" ></textarea>-->
<#--            </div>-->
            <div class="form-group">
                <input type="file" name="phFile" id="phFile" required>
            </div>
            <div class="form-group">
                <br>
                <button class="btn btn-sm btn-secondary" type="button" onclick="startParseClick3()"><i class="fa fa-search fa-lg fa-fw fa-col"></i>图片识别</button>
            </div>
...
<script>
    ...
    function startParseClick3(){
        var formData = new FormData();
        formData.append('file', $('#phFile')[0].files[0]);
        $.ajax({
            type: 'POST',
