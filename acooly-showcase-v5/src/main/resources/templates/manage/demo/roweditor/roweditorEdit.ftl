...
<div>
    <form id="manage_roweditor_editform" class="form-horizontal" action="/manage/demo/roweditor/roweditor/<#if action=='create'>saveJson<#else>updateJson</#if>.html" method="post" >
		<@jodd.form bean="roweditor" scope="request">
        <input name="id" type="hidden" />
		<div class="card-body">
			<div class="form-group row">
				<label class="col-sm-3 col-form-label">会员编码</label>
				<div class="col-sm-9">
					<input type="text" name="memberNo" placeholder="请输入会员编码..." class="easyui-validatebox form-control"  data-options="validType:['text','length[1,32]']" required="true"/>
				</div>
			</div>
			<div class="form-group row">
			...
			<div class="form-group row">
				...
				<div class="col-sm-9">
					<input type="text" name="birthday" placeholder="请输入生日..." class="easyui-validatebox form-control" value="<#if roweditor.birthday??>${roweditor.birthday?string('yyyy-MM-dd')}</#if>" onFocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" data-options="required:true" />
				</div>
			</div>
			<div class="form-group row">
